...
const Verify_Email_on_Signup = () => {
  ...
  const { signUpdata, loading, success, invite_data } = useSelector(
    (state) => state.auth
  );
  ...
  return (
    <>
      <TitleData data={"Mingle-Verify Email"} />
      <div
        className={`${
          darkMode
            ? "bg-gradient-to-br from-black via-richblack-900 to-gray-900"
            : "bg-gradient-to-br from-white via-gray-100 to-gray-300"
        } min-h-screen flex flex-col items-center justify-center p-6 transition-all duration-300`}
      >
        <button
        ...
        <div className="max-w-md p-6 lg:p-8 rounded-xl shadow-2xl backdrop-blur-md bg-white/10 dark:bg-black/30 border border-white/20">
              ...
              <OtpInput
                ...
                renderInput={(props) => (
                  <input
                    {...props}
                    placeholder="-"
                    className={`w-[70px] lg:w-[70px] aspect-square text-center text-lg font-semibold rounded-md focus:outline-none border bg-white/10 ${
                      darkMode
          ...
          <div className="mt-6 flex items-center justify-between text-sm">
            ...
            <button
              className="flex items-center gap-x-2 text-blue-400 hover:text-blue-500"
